		...
		if (#current_replication.items > 0) then
			...
			for _,current_item in ipairs(current_replication.items) do
				...
				if current_item.pointer.icons then
					...
					icon[#icon + 1] = {
						icon = "__dark-matter-replicators-space-age__/graphics/icons/borders/repl-"..icon_name..".png",
						icon_size = 32,
						--scale = max_size / 32
					}
...
--					icon_size = 32,
			...
			if current_replication.overrides.icon then
				...
				tech_icon = {
						...
						{icon = "__dark-matter-replicators-space-age__/graphics/icons/borders/tech-"..current_replication.category.name..".png", icon_size = 128}
				}
			else
