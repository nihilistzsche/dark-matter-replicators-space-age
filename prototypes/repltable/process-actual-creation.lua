	...
	if current_replication.category.enabled then
		...
		for currentItem = #item_table, 1, -1 do
			if item_table[currentItem].pointer == nil then
				log(
					'The item "'
						.. item_table[currentItem].name
						.. '" was removed from the repltech "'
						.. current_replication.name
						.. '" becuase the item in question does not exist.'
				)
		...
		if #current_replication.items > 0 then
			...
			for _, current_item in ipairs(current_replication.items) do
				...
				if type(cost) ~= "number" then
					if type(cost) == "table" then
						log(
							'Something went really wrong when it comes to the replication of "'
								.. current_item.name
								.. "\".  Somehow its cost table could not be processed into a number even though there are enough default cases that this should never be possible.  Maybe a replication is there twice or something.  Here's what the table itself looked like after somehow slipping through cost calculations: "
								.. table.tostring(cost)
						)
						cost = 1
					else
				...
				if current_item.pointer.icon_size then
				...
				if current_item.pointer.icons then
					...
					for _, current_layer in ipairs(current_item.pointer.icons) do
						if current_item.pointer.icons[#icon + 1].icon_size then
							per_icon_size = current_item.pointer.icons[#icon + 1].icon_size
						else
							per_icon_size = icon_size
						end
						--per_icon_scale = current_item.pointer.icons[#icon + 1].icon_scale
						icon[#icon + 1] =
							{ icon = current_layer.icon, icon_size = per_icon_size, scale = per_icon_scale }
						--icon[#icon + 1] = {icon = current_layer.icon, icon_size = icon_size}
					end
					--Add the replication border as an extra layer
					icon[#icon + 1] = {
						icon = "__dark-matter-replicators-space-age__/graphics/icons/borders/repl-"
							.. icon_name
							.. ".png",
						icon_size = 32,
						--scale = max_size / 32
				...
				elseif current_item.pointer.icon then
					...
					log(
						"The item "
							.. current_item.pointer.name
							.. " does not have an icon. Dependency issue? (another mod adds the icon in their data-final-fixes after)"
					)
				end
			...
			if not repldata.icons[1] then
				error("wanted to make a replication tech but there wasn't anything to replicate")
			end
			...
			if current_replication.overrides.icon then
				if current_replication.overrides.icon_size then
					icon_size = current_replication.overrides.icon_size
				else
					icon_size = 128
				end
				tech_icon = {
					...
					{
						icon = "__dark-matter-replicators-space-age__/graphics/icons/borders/tech-"
							.. current_replication.category.name
							.. ".png",
						icon_size = 128,
					},
				}
				...
				tech_icon[#tech_icon].scale = (tech_icon[1].icon_size and tech_icon[1].icon_size / 32 or 1)
			end
			...
			local research_multiplier = current_replication.overrides.research_multiplier
				or current_replication.category.research_multiplier
				or 1
			repl_data[#repl_data + 1] = {
				type = "technology",
				...
				unit = repl_research(current_replication, reps_multiplier * research_multiplier, research_time),
				upgrade = upgrade,
			...
			log(
				'The repltech "'
					.. current_replication.name
					.. '" was not created as a technology becuase it does not contain any replicatable items.'
			)
		end
	end
end
