	...
	for name,item in pairs(unknown_cost_items) do
		...
		for i,current_part in ipairs(item.cost) do
			...
			elseif current_part.type == "recipe" or current_part.type == "recipe-noproduct" then
				...
				if recipe then
					...
					if current_part.type == "recipe" then
						...
						_, multiplier = get_recipe_result_part(recipe)
						if not multiplier then
							log("no multiplier? "..name.." "..amount)
							log(serpent.block(current_part))
						end
			...
			elseif current_part == nil then
				item.cost[i] = 0
			elseif current_part.type == "fluid" then
				--this doesn't work, idk, type = "item" handles it?
				local fluid = current_part.target or current_part.name
				if fluid then
					local cost = known_costs["fluid-"..fluid]
					if cost then
						log(fluid.." cost: "..cost)
						item.cost[i] = cost * (current_part.multiplier or 1)
					else
						log(string.format("Fluid #%d, %s of the replication cost of %s is not known.",
							i, fluid, name))
						item.cost[i] = 0
					end
				else
					log("type was fluid but missing fluid to check: "..name)
					item.cost[i] = 0
				end
			else
