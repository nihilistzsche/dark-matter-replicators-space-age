...
repl_variables = repl_variables or {}
--[[
...
function replsub_recipe(recipe_name, item_overrides)
	...
	if recipe then
		...
		local result, amount = get_recipe_result_part(recipe, nil, item_overrides)
		if not result then
			log("replsub_recipe no result: "..recipe_name)
		end
		--Create and return the subtable
		return {
			name = result,
...
function repltech_element(atomic_number, name, item_table, ore_name, prerequisites, icon_path)
	...
	icon_path = image_path or "__dark-matter-replicators-space-age__/graphics/icons/periodic/"..name..".jpg"
	name = gprefix..name
	prerequisites = replsub_prereq(prerequisites)
	if ore_name then
