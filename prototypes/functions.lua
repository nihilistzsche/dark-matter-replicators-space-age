	...
	for k,v in pairs(results) do
		if (not lookfor) or ((lookfor == v.name) or (lookfor == v[1])) then
			if (v.type == "item") then
				...
				amount = amount or ((v.probability or 1) * .5 * (math.max(v.amount_max, v.amount_min) + v.amount_min))
				--[[
...
--]]
	...
	--	difficulty = difficulty or "normal"
		...
		if difficulty.results then
			...
			if main_product == "" then main_product = nil end

			local result, amount = get_first_thing_from_results(difficulty.results, main_product)
			if not result then
				if needed_result then
					log(recipe.name.." had a specific result but it was not in results, can't be used for replication")
					return nil, nil
		...
		elseif difficulty.result then
			if needed_result and (recipe.result ~= needed_result) then
				log(recipe.name.." was looking for "..needed_result.." but result: "..difficulty.result.." doesn't match")
				return nil, nil
			end

			if (type(difficulty.main_product) == "string") and (difficulty.main_product ~= "") then
				log(recipe.name.." had a main_product but only had result - this is unnecessary and may error other mods")
			end
